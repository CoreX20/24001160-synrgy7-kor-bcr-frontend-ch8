...
async function addCar(requestBody : FormData) {
    try {
        const token = localStorage.getItem("token");
        const response = await fetch("https://condemned-lauretta-bcr-695ec39f.koyeb.app/cars/create", {
            method : "POST",
            headers: {
...
const AdminAddCar: React.FC= () => {
    ...
    return (
        <>
            <Layout>
                <h2 className='fw-bold ms-4'>Add New Car</h2>
                ...
                <div className='d-flex position-absolute ms-2 mb-3 bottom-0' style={{gap:'10px'}}>
                    ...
                    <button type="submit" className="btn btn-primary rounded-0" disabled={submitButtonDisabled} onClick={handleSubmit}>Save</button>
                </div>
